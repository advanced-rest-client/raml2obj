...
class RamlJsonEnhancer {
  ...
  getMeasurements() {
    ...
    performance.measure(ARR2OBJ_MARK,
      ARR2OBJ_MARK + '-start', ARR2OBJ_MARK + '-end');
    performance.measure(EXPAND_MARK,
      EXPAND_MARK + '-start', EXPAND_MARK + '-end');
    performance.measure(NORMALIZE_MARK,
      NORMALIZE_MARK + '-start', NORMALIZE_MARK + '-end');
    performance.measure('making-expansion-consistent',
      'making-expansion-consistent-start',
      'making-expansion-consistent-end');
    let result = [];
    // result.push(
    //   this._measureRersult(ARR2OBJ_MARK, 'Array to object tranformation'));
    // result.push(
    //   this._measureRersult(EXPAND_MARK, 'Datatype expansion'));
    // result.push(
    //   this._measureRersult(NORMALIZE_MARK, 'Data model normalization'));

    // const additional = ExpansionLibrary.getMeasurements();
   ...
   */
  enhance(raml) {
    const perf = this.takeMeasurements;
    if (perf) {
    ...
    if (perf) {
      ...
      this._takeMeasurement(ARR2OBJ_MARK);
    ...
    .then((expanded) => {
      if (perf) {
        ...
        this._takeMeasurement(EXPAND_MARK);
    ...
    .then((data) => {
      ...
      if (perf) {
        ...
        this._takeMeasurement(NORMALIZE_MARK);
        result.measurement = measureData;
      }
      return result;
    });
  }
  ...
  _normalizeObject(raml) {
    ...
    const computeBaseUri = !this.hasProperBaseUri(raml);
    const opts = {
   ...
   * It also applies security schemas definitions on a resource and
   * method level.
   *
   * Affected properties:
  ...
  enhaceJson(ramlObj, rootTypes, opts) {
    ...
    ramlObj.resources.forEach((resource) => {
      ...
      if (opts.allUriParameters) {
        resource.allUriParameters = resource.allUriParameters.concat(
          opts.allUriParameters);
      }

      if (resource.baseUriParameters) {
        // Fixes https://github.com/mulesoft/api-console/issues/136
        resource.allUriParameters =
          this._mergeBaseParameters(resource.allUriParameters,
      ...
      // Copy the RESOURCE uri parameters to the METHOD, because
      // that's where they will be rendered.
      if (resource.methods) {
        resource.methods.forEach((method) => {
          ...
          SecuritySchemesTransformer.addSecuritSchemes(
            method, rootTypes.securitySchemes);
        });
      }
      SecuritySchemesTransformer.addSecuritSchemes(
        resource, rootTypes.securitySchemes);
   ...
   * @return {Boolean} True if the `baseUri` matches first resources
   * `absoluteUri`
   */
  hasProperBaseUri(raml) {
    ...
    const resourceUri = raml.resources[0].absoluteUri;
    if (!resourceUri) {
      return false;
    }
