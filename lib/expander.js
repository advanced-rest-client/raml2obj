...
class ExpansionLibrary {
  ...
  static expandRootTypes(types, measure) {
    ...
    .then((result) => {
      ...
      if (measure) {
        ...
        ExpansionLibrary._takeMeasurement('making-expansion-consistent');
      }
  ...
  /**
   * Recuresively expands types.
   * @param {Array<String>} keys List of keys in the object
   * @param {Object} types Map of types in the API.
   * @param {Boolean} measure If set it produces detailed usage statistics
   * @return {Promise}
   */
  ...
  static _expandType(types, key, measure) {
    return new Promise((resolve) => {
      const library = isNode ? require('datatype-expansion') : expansion;
      if (measure) {
      ...
      library.expandedForm(types[key], types, (err, expanded) => {
        if (err) {
          ...
          if (measure) {
            ...
            ExpansionLibrary._takeMeasurement('expanding-' + key);
          }
          return resolve([key, types[key]]);
        }
        if (expanded) {
          library.canonicalForm(expanded, {
            hoistUnions: false,
            callback: (err2, canonical) => {
              if (measure) {
                ExpansionLibrary.mark('expanding-' + key + '-end');
                ExpansionLibrary._takeMeasurement('expanding-' + key);
              }
